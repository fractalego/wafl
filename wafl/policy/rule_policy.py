...
from wafl.connectors.llm_rule_policy_connector import LLMRulePolicyConnector
...
class RulePolicy:
    def __init__(self, interface, logger=None):
        ...
        self._logger = logger
        self._connector = LLMRulePolicyConnector()
        self._max_num_past_utterances = 3

    async def select(self, rules: List["Rule"], query: "Query") -> List["Rule"]:
        rules_text = ""
        for index, rule in enumerate(rules):
            if rule.effect.text == query.text:
                return [rule]

            rules_text += f"{index + 1}. {rule.effect.text.capitalize()}\n"
