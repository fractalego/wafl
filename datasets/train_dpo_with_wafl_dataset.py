...
original_model = AutoModelForCausalLM.from_pretrained(model_name, load_in_4bit=True)
...
def create_train_dataset(df):
    ...
    max_chosen_length = max(len(tokenizer.encode(chosen_text)) for chosen_text in chosen)
    max_rejected_length = max(len(tokenizer.encode(rejected_text)) for rejected_text in rejected)

    return Dataset.from_dict({"prompt": prompts, "chosen": chosen, "rejected": rejected}), max_prompt_length, max_chosen_length, max_rejected_length


if __name__ == '__main__':
    df = pd.read_parquet("data/wafl_functions.parquet")
    train_dataset, max_prompt_length, max_chosen_length, max_rejected_length = create_train_dataset(df)
    training_args = TrainingArguments(output_dir="./output",
                                      use_cpu=True,
                                      num_train_epochs=0.3, #### Should be 1
    ...
    trainer = DPOTrainer(
        ...
        max_length=max(max_prompt_length + max_chosen_length, max_prompt_length + max_rejected_length),
    )
    trainer.train()
    model.merge_all()
